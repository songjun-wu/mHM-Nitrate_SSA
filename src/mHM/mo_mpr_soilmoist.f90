...
contains
  ...
  subroutine mpr_sm( param , param_SSA, & ! IN:  global parameter set
       ...
       !                       !      hydraulic counductivity for Horizantal flow
       SMs_FC0, nL0_in_L1    & ! OUT: soil moisture deficit from field capacity
       !                       !      w.r.t to saturation
       )
...
use mo_mrm_global_variables, only: L0_L11_Id
    !$  use omp_lib
    ...
    real(dp),    dimension(:),    intent(in)   :: param_SSA 
    ...
    ! Local variables
    integer(i4)                               :: i,ss, kk,pp,qq               ! loop index
...
integer(i4)                          :: ggg
    ...
    integer(i4), dimension(:,:,:),   allocatable     :: mask_3 
    integer(i4), dimension(:,:),   allocatable     :: mask_2
    real(dp), allocatable,  dimension(:)           :: L0_param_SSA
    ...
    ! initialization
    allocate(mask_3(size(ID0), 3, 3))
    do pp = 1, 3
        do qq = 1, 3
            mask_3(:,pp,qq) = ID0
        end do
    end do 
    allocate(mask_2(size(ID0), 3))
    do pp = 1, 3
            mask_2(:,pp) = ID0
    end do 

    thetaS_till  = merge( 0.0_dp, nodata, mask_3 .ne. int(nodata,i4) )
    thetaFC_till = merge( 0.0_dp, nodata, mask_3 .ne. int(nodata,i4) )
    thetaPW_till = merge( 0.0_dp, nodata, mask_3 .ne. int(nodata,i4) )
    thetaS       = merge( 0.0_dp, nodata, mask_2 .ne. int(nodata,i4) )
    thetaFC      = merge( 0.0_dp, nodata, mask_2 .ne. int(nodata,i4) )
    thetaPW      = merge( 0.0_dp, nodata, mask_2 .ne. int(nodata,i4) )
    Ks           = merge( 0.0_dp, nodata, mask_3 .ne. int(nodata,i4) )
    Db           = merge( 0.0_dp, nodata, mask_3 .ne. int(nodata,i4) )
    if( allocated(Ks_non_till) ) Ks_non_till = 0.0_dp
    deallocate(mask_2, mask_3)


if (.not. allocated(L0_param_SSA)) allocate(L0_param_SSA (15142))
do ggg = 1, 185
         L0_param_SSA = merge(param_SSA(ggg),  L0_param_SSA, L0_L11_Id(:).eq. ggg)
end do
    ...
    SELECT CASE(iFlag_soil)
       ...
       CASE(0)
          ...
          do kk = 1, size( soilId0, 1 )
              ss = soilId0(kk,1)
              horizon: do j = 1, nHorizons(ss)
                call hydro_cond( Ks_tmp, param(10:13), sand(ss,j), clay(ss,j), L0_param_SSA(kk) )
                Ks(kk,j,:) = Ks_tmp
                if ( j .le. nTillHorizons(ss) ) then
                   do L = 1, maxval( LCOVER0 )
                      ...
                      pM = 100.0_dp - pOM
                      Db(kk,j,L) = 100.0_dp / ( (pOM/BulkDens_OrgMatter) + (pM/DbM(ss,j)) )


                      ! Effect of organic matter content
                      ! This is taken into account in a simplified form by using
                      ! the ratio of(Bd / BdOM)
                      Ks_tmp = Ks_tmp * ( DbM(ss,j)/Db(kk,j,L) )
                      Ks(kk,j,L) =  Ks_tmp

                      ! estimated SMs_till & van Genuchten's shape parameter (n)
                      call Genuchten( thetaS_till(kk,j,L), Genu_Mual_n, Genu_Mual_alpha, &
                                      param(4:9), sand(ss,j), clay(ss,j), Db(kk,j,L)       )

                      ! estimating field capacity
                      call field_cap( thetaFC_till(kk,j,L), Ks_tmp, thetaS_till(kk,j,L), Genu_Mual_n )
                      ! estimating permanent wilting point
                      call PWP( Genu_Mual_n, Genu_Mual_alpha, thetaS_till(kk,j,L), thetaPW_till(kk,j,L) )
                   end do
                ! deeper layers
                else
                   ! estimate SMs & van Genuchten's shape parameter (n)
                   call Genuchten( thetaS(kk, j-tmp_minSoilHorizon), Genu_Mual_n, Genu_Mual_alpha, &
                                   param(4:9), sand(ss,j), clay(ss,j), DbM(ss,j) )
                   ! estimate field capacity
                   call field_cap( thetaFC(kk,j-tmp_minSoilHorizon), &
                                   Ks_tmp, thetaS(kk,j-tmp_minSoilHorizon), Genu_Mual_n )
                   ! estimate permanent wilting point
                   call PWP( Genu_Mual_n, Genu_Mual_alpha, thetaS(kk, j-tmp_minSoilHorizon), &
                             thetaPW(kk, j-tmp_minSoilHorizon) )
          ...
          cellloop: do i = 1, size( soilId0, 1 ) !>> here = ncells0 
             ...
             do j = 1, nHorizons( s )
                if ( j .le. nTillHorizons( s ) ) then
                   ...
                   KsVar_H0(i) = KsVar_H0(i) + thetaS_till( i, j, LCover0(i) ) * Ks( i, j, LCover0(i) )
                   KsVar_V0(i) = KsVar_V0(i) + thetaS_till( i, j, LCover0(i) ) / Ks( i, j, LCover0(i) )
                   SMs_FC0(i)  = SMs_FC0(i)  + thetaFC_till(i, j, LCover0(i) )
                   SMs_tot0(i) = SMs_tot0(i) + thetaS_till (i, j, LCover0(i) )
                else
                   ! soil_properties over the whole soil column
                   KsVar_H0(i) = KsVar_H0(i)+thetaS(i,j-tmp_minSoilHorizon)*Ks(i,j,1)
                   KsVar_V0(i) = KsVar_V0(i)+thetaS(i,j-tmp_minSoilHorizon)/Ks(i,j,1)
                   SMs_FC0(i)  = SMs_FC0(i) +thetaFC(i,j-tmp_minSoilHorizon)
                   SMs_tot0(i) = SMs_tot0(i)+thetaS (i,j-tmp_minSoilHorizon)
               end if
      ...
      CASE DEFAULT
       ...
       ! free space ** 
       deallocate( SMs_tot0, L0_param_SSA )
  ...
  subroutine hydro_cond( KS, param, sand, clay, L0_param_SSA )
    ...
    real(dp),               intent(in)  :: L0_param_SSA
    ...
    x  =  param(1) +  L0_param_SSA * sand -  param(3) * clay
