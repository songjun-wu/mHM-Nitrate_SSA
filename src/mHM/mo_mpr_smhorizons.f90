...
contains
  ...
  subroutine mpr_SMhorizons( &
       ! Input -----------------------------------------------------------------
       param,param_SSA         , & ! global parameters, three are required
       ...
       CASE(0)
          do h = 1, nHorizons_mHM
             ...
             cellloop: do k = 1, size(LCOVER0,1)
                ...
                Bd0(k) = sum( Db(k,:nTillHorizons(s), L)*Wd(S, H, 1:nTillHorizons(S) ), &
                ...
                SMs0(k) = sum( thetaS_till(k,:nTillHorizons(s), L) &
                     * Wd(S,H, 1:nTillHorizons(S) ), &
                     Wd(S,H, 1:nTillHorizons(S)) > 0.0_dp ) &
                     + sum( thetaS(k,nTillHorizons(S)+1-minval(nTillHorizons(:)):nHorizons(s)-minval(nTillHorizons(:))) &
                     * Wd(S,H, nTillHorizons(S)+1:nHorizons(S)), &
                     Wd(S,H, nTillHorizons(S)+1:nHorizons(S)) > 0.0_dp )
                ! depth weightage FC
                FC0(k) = sum( thetaFC_till(k, :nTillHorizons(s), L) &
                     * Wd(S, H, 1:nTillHorizons(S) ), &
                     Wd(S, H, 1:nTillHorizons(S)) > 0.0_dp ) &
                     + sum( thetaFC(k, nTillHorizons(S)+1-minval(nTillHorizons(:)):nHorizons(s)-minval(nTillHorizons(:))) &
                     * Wd(S, H, nTillHorizons(S)+1:nHorizons(S)), &
                     Wd(S, H, nTillHorizons(S)+1:nHorizons(S)) > 0.0_dp )
                ! depth weightage PWP
                PW0(k) = sum( thetaPW_till(k, :nTillHorizons(s), L) &
                     * Wd(S, H, 1:nTillHorizons(S) ), &
                     Wd(S, H, 1:nTillHorizons(S)) > 0.0_dp ) &
                     + sum( thetaPW(k,nTillHorizons(S)+1-minval(nTillHorizons(:)) :nHorizons(s)-minval(nTillHorizons(:))) &
                     * Wd(S, H, nTillHorizons(S)+1:nHorizons(S)), &
                     Wd(S, H, nTillHorizons(S)+1:nHorizons(S)) > 0.0_dp )
