...
contains
  ...
  subroutine mpr( proc_Mat, & ! IN:    determines which regionalization shall be done
    ...
    case(1)
       ...
       call snow_acc_melt_param( param( iStart:iEnd ), parameterset_SSA(926:1110), c2TSTu,&
    ...
    case(1)
       ...
       allocate(  thetaS_till(size(cell_id0,1) , mtill, mLC) ) 
       allocate( thetaFC_till(size(cell_id0,1) , mtill, mLC) ) 
       allocate( thetaPW_till(size(cell_id0,1) , mtill, mLC) ) 
       allocate(       thetaS(size(cell_id0,1) , mHor      ) ) 
       allocate(      thetaFC(size(cell_id0,1) , mHor      ) ) 
       allocate(      thetaPW(size(cell_id0,1) , mHor      ) )
       allocate(           Ks(size(cell_id0,1) , mHor, mLC ) )
       allocate(           Db(size(cell_id0,1) , mHor, mLC ) ) 
       ...
       call mpr_sm( param(iStart:iEnd),parameterset_SSA(1:185), nodata, iFlag_soil,    &
            ...
            thetaFC, thetaPW, Ks, Db, KsVar_H0, KsVar_V0, SMs_FC0, nL0_in_L1)

       ! next four parameters go here
       ! (the first three for the fRoots and the fourth one for the beta)
       iStart = proc_Mat(3,3) - 4 + 1
       iEnd   = proc_Mat(3,3)
       call mpr_SMhorizons( param(iStart:iEnd),parameterset_SSA(741:925),nodata, iFlag_soil,    &
    ...
    case(0) ! aspect correction of input PET
       ...
       iEnd   = proc_Mat(5,3)    
       call pet_correct( cell_id0, latitude, Asp0, param( iStart : iEnd), parameterset_SSA(186:370), nodata, fAsp0, nL0_in_L1 )
    ...
    case(1) ! Hargreaves-Samani method
       iStart = proc_Mat(5,3) - proc_Mat(5,2) + 1
       iEnd   = proc_Mat(5,3)    
       call pet_correct( cell_id0, latitude, Asp0, param( iStart : iEnd - 1),parameterset_SSA(186:370), nodata, fAsp0, nL0_in_L1 )
       fAsp1 = upscale_arithmetic_mean( nL0_in_L1, Upp_row_L1, Low_row_L1, &
            Lef_col_L1, Rig_col_L1, cell_id0, mask0, nodata, fAsp0 )
    ...
    case (1)
       ...
       call mpr_runoff( LCOVER0, mask0, nodata, SMs_FC0, slope_emp0,        &
            KsVar_H0, param(iStart:iEnd), parameterset_SSA(371:555), cell_id0, upp_row_L1, low_row_L1, &
    ...
    case(1)
       ...
       call karstic_layer(                                                     & ! In
            param( iStart : iEnd ),parameterset_SSA(556:740),                 & ! In
  ...
  subroutine snow_acc_melt_param(                &
       param, param_SSA, c2TSTu, fForest1, fIperm1, fPerm1, &  ! Intent(in)
       TT1, DD1, IDDP1, DDmax1                              &  ! Intent(out)
       )
  ...
  subroutine pet_correct( Id0, latitude_l0, Asp0, param, param_SSA, nodata, fAsp0, nL0_in_L1)
...
use mo_mrm_global_variables, only: L0_L11_Id
    ...
    ! local
    real(dp), dimension(:), allocatable :: L0_param_SSA
    integer(i4)                         :: i, j
integer(i4)                          :: ggg
...
do ggg = 1, 185
    ...
    !$OMP PARALLEL
    fAsp0 = merge(  &
         L0_param_SSA + param(2) / param(3) * asp0, &
         L0_param_SSA + param(2) / (360._dp - param(3)) * (360._dp - Asp0), &
          asp0 < param(3)   )
    fAsp0 = merge( fAsp0, nodata, Id0 /= int(nodata, i4)  )
    !$OMP END PARALLEL

    deallocate(L0_param_SSA)
...
do ggg = 1, 185
    ...
    !$OMP PARALLEL
    tmp = merge( L0_param_SSA * ( 1.0_dp + SMs_FC0 ) / ( 1.0_dp + KsVar_V0 ), &
