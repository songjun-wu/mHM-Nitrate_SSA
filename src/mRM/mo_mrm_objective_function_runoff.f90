...
CONTAINS
  ...
  FUNCTION single_objective_runoff(parameterset,parameterset_SSA)

    USE mo_common_variables, ONLY: opti_function

    IMPLICIT NONE

    REAL(dp), DIMENSION(:), INTENT(IN)  :: parameterset,parameterset_SSA
    REAL(dp)                            :: single_objective_runoff

    select case (opti_function)
    case (1)
       ! 1.0-nse
       single_objective_runoff = objective_nse(parameterset,parameterset_SSA)
    case (2)
       ! 1.0-lnnse
       single_objective_runoff = objective_lnnse(parameterset,parameterset_SSA)
    case (3)
       ! 1.0-0.5*(nse+lnnse)
       single_objective_runoff = objective_equal_nse_lnnse(parameterset,parameterset_SSA)
    case (4)
       ! -loglikelihood with trend removed from absolute errors and then lag(1)-autocorrelation removed
       single_objective_runoff = - loglikelihood_stddev(parameterset,parameterset_SSA, 1.0_dp)
    case (5)
       ! ((1-NSE)**6+(1-lnNSE)**6)**(1/6)
       single_objective_runoff = objective_power6_nse_lnnse(parameterset,parameterset_SSA)
    case (6)
       ! SSE
       single_objective_runoff = objective_sse(parameterset,parameterset_SSA)
    case (7)
       ! -loglikelihood with trend removed from absolute errors
       single_objective_runoff = -loglikelihood_trend_no_autocorr(parameterset,parameterset_SSA, 1.0_dp)
    case (8)
       ! -loglikelihood of approach 2 of Evin et al. (2013),
       !  i.e. linear error model with lag(1)-autocorrelation on relative errors
       single_objective_runoff = -loglikelihood_evin2013_2(parameterset,parameterset_SSA)
    case (9)
       ! KGE
       single_objective_runoff = objective_kge(parameterset,parameterset_SSA)
    case (14)
       ! combination of KGE of every gauging station based on a power-6 norm \n
       ! sum[((1.0-KGE_i)/ nGauges)**6]**(1/6) 
       single_objective_runoff = objective_multiple_gauges_kge_power6(parameterset,parameterset_SSA)
    case default
  ...
  SUBROUTINE multi_objective_runoff(parameterset,parameterset_SSA, multi_objectives) 
    ...
    case (16) 
       ...
       multi_objectives = multi_objective_nse_lnnse(parameterset,parameterset_SSA)
    case (18) 
       ! 1st objective: 1.0 - lnNSE(Q_highflow)  (95% percentile)
       ! 2nd objective: 1.0 - lnNSE(Q_lowflow)   (5% of data range)
       multi_objectives = multi_objective_lnnse_highflow_lnnse_lowflow(parameterset,parameterset_SSA)
    case (19) 
       ! 1st objective: 1.0 - lnNSE(Q_highflow)  (non-low flow)
       ! 2nd objective: 1.0 - lnNSE(Q_lowflow)   (5% of data range)
       multi_objectives = multi_objective_lnnse_highflow_lnnse_lowflow_2(parameterset,parameterset_SSA) 
    case (20) 
       ! 1st objective: 1.0 - difference in FDC's low-segment volume
       ! 2nd objective: 1.0 - NSE of discharge of months DJF
       multi_objectives = multi_objective_ae_fdc_lsv_nse_djf(parameterset,parameterset_SSA) 
    case default 
  ...
  !>        \date Dec 2012
  !         Modified, 
  FUNCTION loglikelihood(parameterset,parameterset_SSA)
    ...
    case (8)
       if (opti_method == 0) then
          ...
          loglikelihood = loglikelihood_evin2013_2(parameterset,parameterset_SSA, regularize=.true.)
       else
          loglikelihood = loglikelihood_evin2013_2(parameterset,parameterset_SSA)
       endif
    case default
  ...
  FUNCTION loglikelihood_stddev(parameterset,parameterset_SSA, stddev, stddev_new, likeli_new)
    ...
    call eval(parameterset,parameterset_SSA, runoff=runoff)
  ...
  FUNCTION loglikelihood_evin2013_2(parameterset,parameterset_SSA, regularize)
    ...
    call eval(parameterset(1:npara-2), parameterset_SSA, runoff=runoff)
  ...
  FUNCTION loglikelihood_trend_no_autocorr(parameterset,parameterset_SSA, stddev_old, stddev_new, likeli_new)
  ...
  FUNCTION objective_lnnse(parameterset,parameterset_SSA)
  ...
  FUNCTION objective_sse(parameterset,parameterset_SSA)
    ...
    call eval(parameterset,parameterset_SSA, runoff=runoff, nutrient = nutrient)
    nGaugesTotal = size(runoff, dim=2)
  ...
  FUNCTION objective_nse(parameterset,parameterset_SSA)
  ...
  FUNCTION objective_equal_nse_lnnse(parameterset,parameterset_SSA)
  ...
  FUNCTION multi_objective_nse_lnnse(parameterset,parameterset_SSA) 
  ...
  FUNCTION multi_objective_lnnse_highflow_lnnse_lowflow(parameterset,parameterset_SSA) 
  ...
  FUNCTION multi_objective_lnnse_highflow_lnnse_lowflow_2(parameterset,parameterset_SSA) 
  ...
  FUNCTION multi_objective_ae_fdc_lsv_nse_djf(parameterset,parameterset_SSA) 
  ...
  FUNCTION objective_power6_nse_lnnse(parameterset, parameterset_SSA)
  ...
  FUNCTION objective_kge(parameterset, parameterset_SSA)
  ...
  FUNCTION objective_multiple_gauges_kge_power6(parameterset, parameterset_SSA)
  ...
  ! ------------------------------------------------------------------
  subroutine eval(parameterset,parameterset_SSA, runoff, nutrient, basin_avg_tws)
    ...
    real(dp), intent(in) :: parameterset_SSA(:)
...
#ifdef mrm2mhm
    call mHM_eval(parameterset,parameterset_SSA, runoff=runoff, nutrient= nutrient, basin_avg_tws=basin_avg_tws)
#else
    call mRM_eval(parameterset,parameterset_SSA, runoff=runoff, nutrient=nutrient)
#endif
