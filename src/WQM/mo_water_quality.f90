...
CONTAINS
  ...
  subroutine wqm(&
    ...
    if (tt == 1)  then
      ...
      !here, rotation type map is modified from land use map due to technical simplification
       ...
       if (perform_mpr) then
          call wqm_mpr(nCells1, nNodes, L1id_on_L11, L11id_on_L1, map_flag, processMatrix, global_parameter, parameterset_SSA,&
               fLAI, fLAI_11, &
    ...
    !in-stream processes
    call conc_routing_process(nNodes, nNodes-1 ,  timeStep, nPerm, nLink_from, nLink_to, nLink_length, &
  ...
  !>        \date Sep 2016
  subroutine conc_routing_process(nNodes, nLinks, TS, nPerm, nLink_from, nLink_to, nLink_length,  &
       nNode_qOUT, nNode_qTR, nNode_concOUT, nNode_interload, nNode_concTIN, nLink_riverbox, nLink_criverbox, nLink_yravg_q, &
  ...
  do k=1, nLinks  
     ...
     call instream_nutrient_processes(TS, iNode, rivertemp11(i), L11_rivert_avg10(i), L11_rivert_avg20(i), &
  ...
  subroutine instream_nutrient_processes(TS, iNode, rivertemp11, rt_avg10, rt_avg20, beneath_area, depth, &
        riverbox, criverbox, aqdenitri, aqassimil, pardeniratew, parprodrate)

		
  implicit none
  integer(i4),                    intent(in)     :: TS, iNode
 ...
 !     INTENT(INOUT)
  ...
  do ir = 1, nCroptation
     if (frac_rotation(ir) > 0.0_dp ) then 
        ...
        ! growing period of winter crops in the next year, before Spring havest 
