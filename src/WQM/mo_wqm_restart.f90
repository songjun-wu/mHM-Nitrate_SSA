...
CONTAINS
  ...
  subroutine wqm_write_restart_files(OutPath )
    ...
    use mo_wqm_global_variables, only: &
         ...
         L11_riverbox, L11_criverbox, L11_yravg_q, L11_concOUT, &
         L11_concTIN, L11_concMod
    ...
    real(dp), dimension(:,:), allocatable    :: dummy_d2
    ...
    basin_loop: do iBasin = 1, size(OutPath)
       ...
       allocate( dummy_d2(nrows1, ncols1))
       dummy_d2 = unpack( L11_riverbox(s1:e1), mask1, nodata_dp )
       var = nc%setVariable("L11_riverbox","f64",(/rows1,cols1/))
       call var%setFillValue(nodata_dp)
       call var%setData(dummy_d2)
       call var%setAttribute("long_name","riverbox in each node, updated by Songjun")

       dummy_d2 = unpack( L11_yravg_q(s1:e1), mask1, nodata_dp )
       var = nc%setVariable("L11_yravg_q","f64",(/rows1,cols1/))
       call var%setFillValue(nodata_dp)
       call var%setData(dummy_d2)
       call var%setAttribute("long_name","annual average discharge, updated by Songjun")
       deallocate(dummy_d2)


       allocate( dummy_d3( nrows1, ncols1, 2 ) )
       do ii = 1, size( dummy_d3, 3 )
          dummy_d3(:,:,ii) = unpack( L11_criverbox(s1:e1,ii), mask1, nodata_dp )
       end do
       var = nc%setVariable("L11_criverbox","f64",(/rows1,cols1,nsubst/))
       call var%setFillValue(nodata_dp)
       call var%setData(dummy_d3)
       call var%setAttribute("long_name","concentration in riverbox, updated by Songjun")

       do ii = 1, size( dummy_d3, 3 )
          dummy_d3(:,:,ii) = unpack( L11_concOUT(s1:e1,ii), mask1, nodata_dp )
       end do
       var = nc%setVariable("L11_concOUT","f64",(/rows1,cols1,nsubst/))
       call var%setFillValue(nodata_dp)
       call var%setData(dummy_d3)
       call var%setAttribute("long_name","updated by Songjun")

       do ii = 1, size( dummy_d3, 3 )
          dummy_d3(:,:,ii) = unpack( L11_concTIN(s1:e1,ii), mask1, nodata_dp )
       end do
       var = nc%setVariable("L11_concTIN","f64",(/rows1,cols1,nsubst/))
       call var%setFillValue(nodata_dp)
       call var%setData(dummy_d3)
       call var%setAttribute("long_name","updated by Songjun")

       do ii = 1, size( dummy_d3, 3 )
          dummy_d3(:,:,ii) = unpack( L11_concMod(s1:e1,ii), mask1, nodata_dp )
       end do
       var = nc%setVariable("L11_concMod","f64",(/rows1,cols1,nsubst/))
       call var%setFillValue(nodata_dp)
       call var%setData(dummy_d3)
       call var%setAttribute("long_name","updated by Songjun")
  ...
  subroutine wqm_read_restart_states( iBasin )
    ...
    use mo_wqm_global_variables, only: &
         L1_humusN, L1_fastN,L1_dissolvedIN,L1_dissolvedON, &
         L1_csoilMoist,L1_cbaseflow,L1_cunsatSTW,L1_cpercolate, &
         nsubstances, L11_riverbox, L11_criverbox, L11_yravg_q, L11_concOUT, &
         L11_concTIN, L11_concMod
    ...
    L11_riverbox(s1:e1) = pack( dummyD2(:,:), mask1)

    var = nc%getVariable("L11_yravg_q")
    call var%getData(dummyD2)
    L11_yravg_q(s1:e1) = pack( dummyD2(:,:), mask1)

  
    var = nc%getVariable("L11_criverbox")
    call var%getData(dummyD3)
    do ii = 1, 2
       L11_criverbox(s1:e1, ii) = pack( dummyD3(:,:, ii), mask1)
    end do
    var = nc%getVariable("L11_concOUT")
    call var%getData(dummyD3)
    do ii = 1, 2
       L11_concOUT(s1:e1, ii) = pack( dummyD3(:,:, ii), mask1)
    end do
    var = nc%getVariable("L11_concTIN")
    call var%getData(dummyD3)
    do ii = 1, 2
       L11_concTIN(s1:e1, ii) = pack( dummyD3(:,:, ii), mask1)
    end do
    var = nc%getVariable("L11_concMod")
    call var%getData(dummyD3)
    do ii = 1, 2
       L11_concMod(s1:e1, ii) = pack( dummyD3(:,:, ii), mask1)
    end do
